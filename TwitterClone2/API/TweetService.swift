...
struct TweetService {
    ...
    func fetchTweets(completion: @escaping([Tweet]) -> Void) {
        ...
        REF_USER_FOLLOWING.child(currentUid).observe(.childAdded) { snapshot in
            let followingsUid = snapshot.key
            REF_USER_TWEETS.child(followingsUid).observe(.childAdded) { snapshot in
    ...
    func fetchReplies(forUser user: User, completion: @escaping([Tweet]) -> Void) {
        ...
        REF_USER_REPLIES.child(user.uid).observe(.childAdded) { snapshot in
            ...
            REF_TWEET_REPLIES.child(tweetKey).child(replyKey).observeSingleEvent(of: .value) { snapshot in
                ...
                UserSevice.shared.fetchUser(uid: uid) { user in
                    let reply : Tweet = Tweet(user: user,tweetID: replyID, dictionary: dictionary)
    ...
    func checkIfUserLikedTweet(_ tweet: Tweet, completion: @escaping(Bool) -> Void) {
        guard let uid = Auth.auth().currentUser?.uid else {return}
        REF_USER_LIKES.child(uid).child(tweet.tweetID).observeSingleEvent(of: .value) { snap in
